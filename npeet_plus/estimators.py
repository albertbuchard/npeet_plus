...
def mi_confidence_interval(
    ...
    mi_bootstrap : ndarray, shape (n_bootstraps,)
        MI values from the bootstrap distribution.
    """
    np.random.seed(random_state)
    ...
    n_samples = len(x)
    assert n_samples == len(y), "x and y must have the same length."
    if z is not None:
        assert n_samples == len(z), "z must have the same length as x and y."
    ...
    # Initialize the array to store MI values from bootstraps
    mi_bootstrap = np.zeros(n_bootstraps)
    ...
    for i in range(n_bootstraps):
        ...
        mi_boot = mi_func(x_resampled, y_resampled, z=z_resampled, **mi_kwargs)
        mi_bootstrap[i] = mi_boot

    # Compute the confidence interval
    alpha = 1 - confidence_level
    lower_bound = np.percentile(mi_bootstrap, 100 * (alpha / 2))
    upper_bound = np.percentile(mi_bootstrap, 100 * (1 - alpha / 2))

    return mi_observed, lower_bound, upper_bound, mi_bootstrap
...
def shuffle_test(measure, x, y, z=False, ns=200, ci=0.95, **kwargs):
    """Shuffle test
    The function you've provided aims to perform a shuffle test to estimate
     the distribution of a statistical measure (like mutual information)
    ...
    ----------
    measure : function
        The statistical measure to evaluate.
    x : array-like
        First variable.
    y : array-like
        Second variable.
